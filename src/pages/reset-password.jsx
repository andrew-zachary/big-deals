    ...
    return <div id="reset-password-page" className="h-full flex flex-col items-center justify-center">
        { 
            btnTxt !== 'continue' && <>
                ...
                <button type='button' className='font-ssp font-regular text-4xl bg-primary shadow-secondary text-white px-8 py-4 rounded-[0.4rem] capitalize mt-8' onClick={()=>resettingPassword()}>{btnTxt}</button>
            </>
        }
        {
            btnTxt === 'continue' && <>
                <p className="px-4 text-4xl font-mont font-regular text-center capitalize">you can now use your new password to sign in</p>
                <Link type='button' className='font-ssp font-regular text-4xl bg-primary shadow-secondary text-white px-8 py-4 rounded-[0.4rem] capitalize mt-8' to="/shopping">{btnTxt}</Link>
            </>
        }
    </div>
}
