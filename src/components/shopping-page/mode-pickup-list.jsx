...
const ModePickupList = ({showModeList, setPickedMode}) => {
    ...
    return <>
        {
            showModeList === 'show' && <ul id="mode-pickup-list" className="w-full block border-primary dark:border-primary-dark border-x border-b-4">
                {
                    pickingModes.map(mode => {
                        return <li key={mode.value} className="cursor-pointer mt-4 capitalize hover:bg-primary dark:bg-primary-dark hover:text-white" onClick={()=>setPickedMode(mode.value)}>
